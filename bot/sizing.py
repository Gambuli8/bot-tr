...
def build_plan_fixed_risk(
    *,
    ...
    """
    Riesgo FIJO por operación: la cantidad sale de cuánto se pierde si toca el SL
    (distancia al SL + comisiones de entrada y salida) = risk_usdt.
    Se descarta si con ese riesgo no se llega al mínimo del contrato.
    El apalancamiento es el máximo que mantiene la liquidación lejos del SL (margen mínimo).
    """
    base = TradePlan(ok=False, reason="", symbol=symbol, direction=direction,
                     entry=entry, stop_loss=stop_loss, take_profit=take_profit)
    if direction == "LONG" and not (stop_loss < entry < take_profit):
    ...
    sl_dist = abs(entry - stop_loss) / entry
    fee_per_unit = entry * spec.taker_fee * 2
    qty = round_step(risk_usdt / (abs(entry - stop_loss) + fee_per_unit), spec.qty_precision)
    if qty < Decimal(str(spec.min_qty)) or float(qty) * entry < spec.min_usdt:
        needed = max(spec.min_qty, spec.min_usdt / entry) * (abs(entry - stop_loss) + fee_per_unit)
        base.reason = (f"con {money(risk_usdt)} de riesgo no llego al mínimo de {spec.symbol} "
                       f"(haría falta arriesgar {money(needed)})")
        return base

    safe_lev = int(1 / (sl_dist / LIQ_SAFETY + mmr))
    leverage = max(1, min(max_leverage, safe_lev))
    liq = estimate_liquidation(entry, leverage, direction, mmr)
    if sl_dist >= abs(entry - liq) / entry * LIQ_SAFETY:
        base.reason = f"el SL está a {pct(sl_dist * 100)}: demasiado lejos para operar con margen aislado"
        return base
    ...
    plan = TradePlan(
        ...
        tp_str=format(round_step(take_profit, spec.price_precision, ROUND_HALF_UP), "f"),
        notional=notional, margin_used=notional / leverage,
        sl_distance_pct=sl_dist * 100, tp_distance_pct=abs(take_profit - entry) / entry * 100,
        risk_usdt=risk, reward_usdt=reward, fees_usdt=fees, rr=rr, liquidation_price=liq,
    )
    if rr < min_rr:
        plan.ok = False
